...
def write_table(df, snowflake_database, snowflake_schema, snowflake_table, if_exists='replace'):
    ...
    with engine.connect() as con:
        df.to_sql(name=snowflake_table.lower(), con=con, if_exists=if_exists, index=False)
        ...
        for column_name, column_comment in column_comments.items():
            ...
            if matching_columns:
                ...
                if column_description:
                    ...
                    new_description = f"{column_comment}"
                else:
...
def transform_figures_on_aviation(df):
    df = pd.DataFrame(df)
    df = df.drop('ID', axis=1)
    # check if these values still exist in the dataframe
        # df[df.eq("       .").any(axis=1)]
    df = df.replace("       .",pd.NA)

    return df

def transform_income_households(df):
    df = pd.DataFrame(df)
    df = df.drop('ID', axis=1)
    # check if these values still exist in the dataframe
        # df[df.eq("       .").any(axis=1)]
